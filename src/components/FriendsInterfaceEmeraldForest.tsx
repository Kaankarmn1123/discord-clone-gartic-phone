// src/components/FriendsInterfaceEmeraldForest.tsx
...
const FriendsInterfaceEmeraldForest: React.FC<FriendsInterfaceProps> = ({ onServerAction, agora, onStartDm }) => {
    ...
    return (
        <div className="flex flex-grow min-w-0">
            <div className="flex flex-col flex-grow">
                <div className="flex items-center p-6 bg-emerald-950/70 border-b-2 border-emerald-500/30 shadow-2xl flex-shrink-0 backdrop-blur-xl relative">
                    <div className="absolute inset-0 opacity-20"><div className="absolute top-0 left-1/4 w-32 h-full bg-gradient-to-b from-emerald-400/20 to-transparent blur-3xl animate-sway-light"></div></div>
                    <div className="flex items-center space-x-6 relative z-10">
                        <div className="flex items-center bg-emerald-950/50 border-2 border-emerald-500/40 px-5 py-2.5 rounded-2xl">
                            <svg className="w-7 h-7 mr-3 text-emerald-400" fill="currentColor" viewBox="0 0 24 24"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>
                            <span className="text-emerald-200 font-serif tracking-wide">{tr.friends}</span>
                        </div>
                        <div className="h-10 w-0.5 bg-emerald-500/40 rounded-full"></div>
                        <button onClick={() => setActiveTab('all')} className={`px-5 py-2 font-semibold tracking-wide text-sm transition-all duration-300 rounded-lg ${activeTab === 'all' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/30' : 'bg-emerald-950/40 text-emerald-300 hover:bg-emerald-900/60 hover:text-white'}`}>{tr.allFriends}</button>
                        <button onClick={() => setActiveTab('pending')} className={`px-5 py-2 font-semibold tracking-wide text-sm transition-all duration-300 rounded-lg ${activeTab === 'pending' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/30' : 'bg-emerald-950/40 text-emerald-300 hover:bg-emerald-900/60 hover:text-white'}`}>{tr.pending}</button>
                        <button onClick={() => setActiveTab('add')} className="px-6 py-2.5 font-bold text-sm text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 rounded-lg shadow-lg shadow-green-500/40 transition-all duration-300 hover:scale-105">{tr.addFriend}</button>
                    </div>
                </div>
                <div className="flex-grow flex flex-col min-w-0 bg-gradient-to-br from-emerald-950 via-green-950 to-slate-950 relative overflow-hidden">
                    <div className="absolute inset-0 opacity-10"><div className="absolute top-0 left-1/4 w-32 h-full bg-gradient-to-b from-emerald-400/10 to-transparent blur-3xl animate-sway-light-slow"></div></div>
...
const listItemClasses = "flex items-center justify-between p-4 bg-emerald-950/40 border-l-4 border-emerald-900/50 hover:border-emerald-500 transition-all duration-300 backdrop-blur-xl rounded-r-2xl shadow-lg hover:shadow-emerald-950/50 group";
...
const AllFriendsTab = ({ onStartDm }: { onStartDm: (friend: Friend) => void }) => {
    ...
    if (friends.length === 0) return <div className="flex-grow flex items-center justify-center p-12 text-center text-emerald-300/70 text-lg font-light italic">{tr.noFriendsYet}</div>;

    return (
        <div className="p-8 overflow-y-auto">
            <div className="flex items-center mb-6 border-b-2 border-emerald-500/20 pb-4"><h3 className="text-sm font-semibold tracking-wide text-emerald-300 font-serif">{tr.allFriends}</h3><div className="ml-3 px-3 py-1 bg-emerald-500/20 text-emerald-200 text-xs font-semibold rounded-full border border-emerald-500/30">{friends.length}</div></div>
            <div className="space-y-3">
                {friends.map(friend => (
                    <div key={friend.id} className={listItemClasses}>
                        <div className="flex items-center"><div className="relative mr-4"><div className="absolute -inset-1 bg-emerald-400 blur-sm rounded-full opacity-0 group-hover:opacity-20 transition-opacity duration-300"></div><img src={friend.avatar_url || `https://robohash.org/${friend.id}.png?set=set1&size=40x40`} alt={friend.username} className="w-12 h-12 rounded-full relative border-2 border-emerald-800/50 group-hover:border-emerald-400/50 transition-all duration-300"/><StatusIndicator status={friend.status} className="absolute -bottom-1 -right-1 ring-2 ring-emerald-950"/></div><span className="font-semibold text-emerald-100 group-hover:text-emerald-200 transition-colors duration-200 font-serif">{friend.username}</span></div>
                        <div className="flex items-center space-x-2">
                            <button onClick={() => onStartDm(friend)} className="p-2.5 text-emerald-300 border border-transparent hover:bg-emerald-500/10 hover:border-emerald-500/20 rounded-xl transition-all duration-200" aria-label={`Message ${friend.username}`}><MessageIcon /></button>
                            <button onClick={() => handleRemoveFriend(friend.friendship_id)} className="p-2.5 text-emerald-300 border border-transparent hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-400 rounded-xl transition-all duration-200" aria-label={`Remove ${friend.username}`}><TrashIcon /></button>
                        </div>
                    </div>
                ))}
            </div>
...
const PendingRequestsTab = ({ onAction }: { onAction: () => void }) => {
    ...
    if (loading) return <div className="flex-grow flex items-center justify-center"><Spinner /></div>;
    if (pendingRequests.length === 0 && serverInvites.length === 0 && joinRequests.length === 0) return <div className="flex-grow flex items-center justify-center p-12 text-center text-emerald-300/70 text-lg font-light italic">{tr.noPendingRequests}</div>;
...
const AddFriendTab = () => {
    ...
    return (
        <div className="p-12">
            <div className="max-w-2xl"><h3 className="text-2xl font-serif text-emerald-200 mb-3">{tr.addFriend}</h3><p className="text-green-300/80 mb-8 italic">{tr.addFriendDescription}</p>
                <form onSubmit={handleSendRequest} className="border-t-2 border-emerald-500/20 pt-8">
                     <div className="relative group">
                        <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-600 to-green-600 rounded-xl blur opacity-0 group-hover:opacity-75 transition duration-500"></div>
                        <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder={tr.enterUsername} className="relative w-full text-white bg-emerald-950/50 p-4 pr-44 rounded-xl border-2 border-emerald-500/30 focus:border-green-400 focus:ring-2 focus:ring-green-500/30 transition-all duration-300 text-base backdrop-blur-sm" />
                        <button type="submit" disabled={loading || !username.trim()} className="absolute right-2 top-2 bottom-2 px-6 text-sm font-bold text-white rounded-lg disabled:bg-slate-700 disabled:cursor-not-allowed bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 shadow-lg shadow-emerald-500/30 transition-all duration-300 hover:scale-105">{loading ? `${tr.sending}...` : tr.sendRequest}</button>
                    </div>
                </form>
                {message && <p className="mt-8 text-sm text-center text-emerald-200 bg-emerald-950/40 backdrop-blur-sm px-6 py-4 rounded-xl border border-emerald-500/20 italic">{message}</p>}
            </div>
        </div>
    );
};

export default FriendsInterfaceEmeraldForest;
