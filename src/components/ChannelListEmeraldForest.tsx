// components/ChannelListEmeraldForest.tsx
...
const TextChannelIcon = () => (
  <svg className="w-5 h-5 mr-2 text-emerald-400 group-hover:text-green-300 transition-colors duration-300" fill="currentColor" viewBox="0 0 20 20">
...
const VoiceChannelIcon = () => (
  <svg className="w-5 h-5 mr-2 text-green-400 group-hover:text-emerald-300 transition-all duration-300 group-hover:scale-110" fill="currentColor" viewBox="0 0 20 20">
...
const ChannelListEmeraldForest: React.FC<ChannelListProps> = ({ server, onChannelSelect, activeChannelId, onServerUpdate, agora, showLegacyVoiceControls, isCollapsed, onToggle }) => {
        ...
        <>
            <div className="relative h-full flex-shrink-0">
                <div className={`h-full transition-all duration-500 ease-in-out overflow-hidden ${isCollapsed ? 'w-0' : 'w-64'}`}>
                    <div className={`w-64 h-full flex flex-col ${theme.colors.bgSecondary} border-r border-emerald-900/30 shadow-2xl relative`}>
                        {/* Organic Forest Background */}
                        <div className="absolute inset-0 bg-gradient-to-br from-emerald-950 via-green-950 to-slate-950 pointer-events-none"></div>
                        ...
                        {/* Server Header - Natural Style */}
                        <div className="relative p-4 border-b border-emerald-800/40 shadow-lg bg-gradient-to-b from-emerald-900/20 to-transparent flex-shrink-0">
                            <button onClick={() => setDropdownOpen(!isDropdownOpen)} 
                                    className="w-full flex items-center justify-between text-white hover:bg-emerald-900/20 px-4 py-3 rounded-2xl transition-all duration-300 hover:shadow-lg font-medium tracking-wide">
                            ...
                            {isDropdownOpen && (
                                <div className={`absolute top-full left-0 right-0 mt-2 mx-2 p-2 ${theme.colors.bgTertiary} rounded-2xl shadow-2xl z-50 text-sm border border-emerald-500/30 backdrop-blur-xl`}>
                                    <button onClick={() => { setInviteModalOpen(true); setDropdownOpen(false); }} 
                                            className={`w-full text-left px-4 py-2.5 ${theme.colors.text} ${theme.colors.textHover} rounded-lg transition-all duration-300 flex items-center gap-3 font-medium`}>
                                    ...
                                    {isOwner && (<>
                                        <button onClick={() => { setServerSettingsModalOpen(true); setDropdownOpen(false); }} 
                                                className={`w-full text-left px-4 py-2.5 ${theme.colors.textSecondary} hover:${theme.colors.textPrimary} rounded-lg hover:${theme.colors.activeBackground} transition-all duration-300 flex items-center gap-3 font-medium mt-1`}>
                                            <SettingsIcon />
                                            {tr.serverSettings}
                                        </button>
                        ...
                        {/* Channels List */}
                        <div className="flex-grow p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-emerald-500/30 scrollbar-track-transparent hover:scrollbar-thumb-emerald-400/50">
                        ...
                        {/* Bottom Panels */}
                        <div className="flex-shrink-0 border-t border-emerald-900/30">
                ...
                <button
                    onClick={onToggle}
                    className={`absolute top-1/2 -translate-y-1/2 w-8 h-14 bg-gradient-to-br from-emerald-800/90 to-green-900/90 hover:from-emerald-600 hover:to-green-600 rounded-r-2xl z-20 flex items-center justify-center border-y-2 border-r-2 border-emerald-500/40 hover:border-emerald-400/60 shadow-[5px_0_20px_rgba(16,185,129,0.2)] hover:shadow-[5px_0_30px_rgba(16,185,129,0.4)] group transition-all duration-300 ease-in-out backdrop-blur-sm ${isCollapsed ? 'left-0' : 'left-64'}`}
                    aria-label={isCollapsed ? "Kanal listesini aç" : "Kanal listesini kapat"}
                >
                    <svg className={`w-5 h-5 text-emerald-300 group-hover:text-white transition-all duration-300 ${isCollapsed ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
...
const ChannelCategory: React.FC<ChannelCategoryProps> = ({ title, channels, activeChannelId, onChannelClick, isOwner, setSelectedChannel, setEditChannelModalOpen, agora }) => {
    ...
    return (
        <div className="mb-8">
            <h3 className={`px-2 mb-3 text-xs font-semibold tracking-wider uppercase ${theme.colors.textPrimary} opacity-60`}>{title}</h3>
                        ...
                        <div key={channel.id}>
                            <div className="group relative">
                                <button
                                    ...
                                    className={`w-full flex items-center px-3 py-2 text-left rounded-lg transition-all duration-300 border-l-4 relative overflow-hidden font-medium
                                    ${isActive
                                        ? `${theme.colors.bgPrimary} ${theme.colors.textPrimary} border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.15)]`
                                        : `${theme.colors.bgSecondary} ${theme.colors.textMuted} border-transparent hover:bg-emerald-900/30 hover:text-slate-100`
                                    }`}
                                >
                                    {isActive && (
                                ...
                                {isOwner && (
                                    <button 
                                        ...
                                        className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 ${theme.colors.textMuted} rounded-lg opacity-0 group-hover:opacity-100 hover:bg-emerald-600/50 hover:text-white transition-all duration-300`}>
                                        <SettingsIcon />
                                    </button>
                                )}
                            </div>
...
export default ChannelListEmeraldForest;
