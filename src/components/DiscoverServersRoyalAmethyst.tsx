// components/DiscoverServersRoyalAmethyst.tsx
...
const DiscoverServersRoyalAmethyst: React.FC<DiscoverServersProps> = ({ onServerJoined }) => {
    ...
    const handleJoinAction = async (server: DiscoverServer) => {
        ...
        if (server.requires_approval) {
             await supabase.from('server_invites').insert({ server_id: server.id, inviter_id: server.owner_id, invitee_id: user.id, status: 'pending', type: 'join_request' });
        } else {
    ...
    return (
        <div className="flex flex-col flex-grow min-h-0 bg-gradient-to-br from-slate-900 via-purple-900 to-fuchsia-950 p-8 overflow-y-auto relative">
            <div className="absolute inset-0 opacity-5"><div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, #a855f7 1px, transparent 0)', backgroundSize: '32px 32px' }}></div></div>
            ...
            <div className="relative z-10">
                <div className="mb-10">
                    <div className="flex items-center space-x-4 mb-3">
                        ...
                        <h1 className="text-4xl font-bold bg-gradient-to-r from-white via-purple-100 to-fuchsia-100 bg-clip-text text-transparent drop-shadow-lg">{tr.discoverServers}</h1>
                    </div>
