// src/components/FriendsInterfaceClassicIndigo.tsx
...
const PendingRequestsTab = ({ onAction }: { onAction: () => void }) => {
    const { user } = useAuth();
    const { pending: pendingRequests, loading: friendsLoading, refetch: refetchFriends } = useFriends();
    ...
    const fetchInvites = useCallback(async () => {
        ...
        try {
            const [{ data: inviteData }, { data: ownedServersData }] = await Promise.all([
            ...
            if (ownedServersData && ownedServersData.length > 0) {
                ...
                if (joinRequestsData) { const validJoinRequests = joinRequestsData.map(invite => { const server = Array.isArray(invite.servers) ? invite.servers[0] : invite.servers; const profile = Array.isArray(invite.profiles) ? invite.profiles[0] : invite.profiles; return server && profile ? { ...invite, servers: server, profiles: profile } : null; }).filter((i): i is ServerInvite => i !== null); setJoinRequests(validJoinRequests); }
    ...
    useEffect(() => {
        ...
        const sub = supabase.channel(`friends-interface-pending-invites`).on('postgres_changes', { event: '*', schema: 'public', table: 'server_invites' }, fetchInvites).subscribe();
        return () => { supabase.removeChannel(sub); }
    }, [user?.id, fetchInvites]);

    const handleAcceptFriend = async (friendshipId: string) => { await supabase.from('friendships').update({ status: 'accepted' }).eq('id', friendshipId); refetchFriends(); };
    const handleDeclineFriend = async (friendshipId: string) => { await supabase.from('friendships').delete().eq('id', friendshipId); refetchFriends(); };
