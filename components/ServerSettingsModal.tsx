...
const MembersTab: React.FC<{ server: Server }> = ({ server }) => {
    ...
    const fetchMembers = useCallback(async () => {
        ...
        if (membershipError) {
          console.error("Error fetching member IDs in settings:", membershipError);
          setMembers([]);
          setLoading(false);
          return;
        }
        ...
        if (profileError) {
          console.error("Error fetching member profiles in settings:", profileError);
          setMembers([]);
        } else if (profileData) {
            const memberProfiles = (profileData as Profile[]).map(profile => ({
                ...profile,
                user_id: profile.id
            }));
