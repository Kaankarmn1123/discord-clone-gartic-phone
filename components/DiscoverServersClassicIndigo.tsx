...
const DiscoverServersClassicIndigo: React.FC<DiscoverServersProps> = ({ onServerJoined }) => {
    ...
    const fetchDiscoverableServers = useCallback(async () => {
        ...
        // Fetch user's friends, memberships, and pending join requests in parallel for efficiency
        const [
        ...
        // Fetch from the pre-joined 'discoverable_servers' view for performance and simplicity
        const serverQuery = supabase
            .from('discoverable_servers')
            .select('*');
            
        // Fetch public servers AND servers owned by friends
    ...
    const handleJoinAction = async (server: DiscoverServer) => {
        ...
        if (server.requires_approval) {
            ...
            const { error } = await supabase.from('server_invites').insert({
                server_id: server.id,
                inviter_id: server.owner_id, // The owner is the one to approve
                invitee_id: user.id,
                status: 'pending',
                type: 'join_request'
            });
...
// FIX: Correct prop type for joiningServerId
