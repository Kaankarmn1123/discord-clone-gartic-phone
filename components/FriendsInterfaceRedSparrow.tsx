    ...
    return (
        <div className="flex flex-grow min-w-0">
            <div className="flex flex-col flex-grow">
                {/* RED SPARROW: Cyberpunk header with glitch effects */}
...
const AllFriendsTab = ({ onStartDm }: { onStartDm: (friend: Friend) => void }) => {
    ...
    useEffect(() => {
        fetchData();
        const sub = supabase.channel(`friends-interface-all-friends`).on('postgres_changes', { event: '*', schema: 'public', table: 'friendships' }, fetchData).subscribe();
...
const PendingRequestsTab = ({ onAction }: { onAction: () => void }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        } else { setJoinRequests([]); } setLoading(false);
    }, [user]);

    useEffect(() => {
        fetchData();
        const sub = supabase.channel(`friends-interface-pending`).on('postgres_changes', { event: '*', schema: 'public', table: 'friendships' }, fetchData).on('postgres_changes', { event: '*', schema: 'public', table: 'server_invites' }, fetchData).subscribe();
        return () => { supabase.removeChannel(sub); }
    }, [fetchData]);
