...
const MemberListClassicIndigo: React.FC<MemberListProps> = ({ serverId, serverOwnerId }) => {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    setLoading(true);
    const { data: membershipData, error: membershipError } = await supabase.from('memberships').select('user_id').eq('server_id', serverId);
    if (membershipError) { console.error("Error fetching member IDs:", membershipError); setMembers([]); setLoading(false); return; }
    const memberIds = membershipData.map(m => m.user_id);
    if (memberIds.length === 0) { setMembers([]); setLoading(false); return; }
    const { data: profileData, error: profileError } = await supabase.from('profiles').select('*').in('id', memberIds);
    if (profileError) { console.error("Error fetching member profiles:", profileError); setMembers([]); } 
    else if (profileData) {
      const sortedProfiles = (profileData as Profile[]).sort((a, b) => a.username.localeCompare(b.username));
