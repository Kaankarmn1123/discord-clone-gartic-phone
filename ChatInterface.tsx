...
import GameContainerModal from './components/games/GameContainerModal';
...
const FriendsInterface = React.lazy(() => import('./components/FriendsInterface'));
const AdminPanel = React.lazy(() => import('./components/AdminPanel'));
const DiscoverServers = React.lazy(() => import('./components/VoiceChannelUI'));
const ChannelList = React.lazy(() => import('./components/ChannelList'));
const MessageView = React.lazy(() => import('./components/MessageView'));
const MemberList = React.lazy(() => import('./components/MemberList'));
...
const ChatInterfaceRedSparrow: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const gameSessionSubscription = supabase.channel(`game-session-${channelId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const renderCurrentView = () => {
    switch(currentView) {
      case 'friends':
        return <FriendsInterface onServerAction={refetchAll} agora={agora} onStartDm={handleStartDm} />;
      case 'admin':
        return <AdminPanel />;
      case 'discover':
        return <DiscoverServers onServerJoined={refetchAll} />;
      case 'server':
        if (activeServer) {
          return (
            <>
        ...
        if (activeChannel && dmRecipient) {
          return (
            <div className="flex flex-col flex-grow min-w-0 border-l-2 border-red-900">
              ...
              <main className={`flex flex-col flex-grow min-w-0 ${theme.colors.bgPrimary} relative`}><div className="relative z-10 flex flex-col flex-grow min-h-0"><MessageView key={activeChannel.id} channel={activeChannel} onOpenGameLauncher={() => setIsGameSelectionModalOpen(true)} onJoinGame={handleJoinGame} activeGameSession={activeChannelGameSession} /></div></main>
            </div>
          );
        }
        return null;
